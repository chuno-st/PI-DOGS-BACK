            ...
            <form className={s.container} onSubmit={handleOnSubmit}>
                <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="height_min" value={state.height_min} onChange={handleChange} placeholder='Altura mínima (centímetros)' required />
                        {state.errors.height_min && <p className="">{state.errors.height_min}</p>}
                    </div>
                    <div>
                        <label>{state.height_max && <>{'Altura máxima (cm) : '}</>}</label>
                        <input type="text" name="height_max" value={state.height_max} onChange={handleChange} placeholder='Altura máxima (centímetros)' required />
                        {state.errors.height_max && <p>{state.errors.height_max}</p>}
                    </div>
                    <div>
                        <label>{state.weight_min && <>{'Peso mínimo (kg) : '}</>}</label>
                        <input type="text" name="weight_min" value={state.weight_min} onChange={handleChange} placeholder='Peso mínimo (kilogramos)' required />
                        {state.errors.weight_min && <p>{state.errors.weight_min}</p>}
                    </div>
                    <div>
                        <label>{state.weight_max && <>{'Peso máximo (kg) : '}</>}</label>
                        <input type="text" name="weight_max" value={state.weight_max} onChange={handleChange} placeholder='Peso máximo (kilogramos)' required />
                        {state.errors.weight_max && <p>{state.errors.weight_max}</p>}
                    </div>
                    <div>
                        <label>{state.life_span_min && <>{'Esperanza de vida mínima (años) : '}</>}</label>
                        <input type="text" name="life_span_min" value={state.life_span_min} onChange={handleChange} placeholder='Esperanza de vida mínima (años)' />
                        {state.errors.life_span_min && <p>{state.errors.life_span_min}</p>}
                    </div>
                    <div>
                        <label>{state.life_span_max && <>{'Esperanza de vida máxima (años) : '}</>}</label>
                        <input type="text" name="life_span_max" value={state.life_span_max} onChange={handleChange} placeholder='Esperanza de vida máxima (años)' />
                        {state.errors.life_span_max && <p>{state.errors.life_span_max}</p>}
                    </div>
                    <label>Seleccionar temperamento/s:
                        <br />
                        <select name='temperaments' value={state.value} onChange={handleSelectChange}>
                        ...
                        <input type="button" value="Agregar" onClick={agregaTemperamento} />
                        {temperaments && temperaments.map((e, i) => (
                            <div key={i}>
                                <label htmlFor={`${i}`}>{`Temperamento  # ${i + 1} :  ${e.temperament}`}</label>
                            </div>
                        ))}
                    </label>
                    <br />
